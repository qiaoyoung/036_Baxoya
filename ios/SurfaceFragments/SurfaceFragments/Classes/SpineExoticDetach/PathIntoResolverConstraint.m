...
//: @implementation PathIntoResolverConstraint
@implementation PathIntoResolverConstraint

//: + (unsigned long long)fileSize:(NSString *)filepath
+ (unsigned long long)drawer:(NSString *)filepath
{
...
+ (NSString *)border:(NSString *)filePath
{
...
+ (NSString *)wiseEvolution:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self drawer:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self border:filepath];
        //: }else {
        }else {
...
+ (void)story:(NSString *)filepath modelThroughBack:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self wiseEvolution:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
