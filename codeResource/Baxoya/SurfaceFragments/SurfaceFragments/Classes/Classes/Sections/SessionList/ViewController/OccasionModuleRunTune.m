...
@property (nonatomic, strong) ConstructConvertView *dropdownMenu;/** 下拉菜单 */
...
-(void)checkCreateTeam{
    ...
    [ElevatedNativeRecoverAllocator getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
- (NSArray *)getMenuModelsArray {    
    ...
    ConstructConvertModel *menuModel1 = [ConstructConvertModel ff_DropDownMenuModelWithMenuItemTitle:activity menuItemIconName:@"ic_create_chat" menuBlock:^{
        if (_creatTeam.integerValue != 0) {
            [FontVentureOrderly showMessage:LangKey(@"please_contact_your_administrator")];
        }else{
    ...
    //菜单模型1
    @weakify(self);
    ConstructConvertModel *menuModel3 = [ConstructConvertModel ff_DropDownMenuModelWithMenuItemTitle:LangKey(@"fragment_contact_new_scan") menuItemIconName:@"ic_scan" menuBlock:^{
        @strongify(self);
        [self requestAuthorizationForVideo];
    }];
...
-(void)newGroupSyncRequest:(NSString *)groupName teamID:(NSString *)teamId{
    ...
    dict[@"name"] = groupName;
    dict[@"id"] = teamId;
    [ElevatedNativeRecoverAllocator getWithUrl:Server_team_create params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_noticeView){
        _noticeView = [[AlongVibratingGloriousFlowBracket alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
...
{
    if(!_loadingView){
        _loadingView = [[WithoutCollateStrategyAuthenticate alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_policyView){
        _policyView = [[VariantFriendlySystematicSmart alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
