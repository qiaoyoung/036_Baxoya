...
- (void)remoteUpdateGender{
    ...
    [ElevatedNativeRecoverAllocator getWithUrl:Server_user_editinfo params:dict isShow:YES success:^(id responseObject) {
...
- (PatternVibratingCircuitStage *)changeNickNameView
{
    if(!_changeNickNameView){
        _changeNickNameView = [[PatternVibratingCircuitStage alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_sexView){
        _sexView = [[TransitionNotebookDispatch alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
//        _sexView.hidden = YES;
    }
    return _sexView;
}
