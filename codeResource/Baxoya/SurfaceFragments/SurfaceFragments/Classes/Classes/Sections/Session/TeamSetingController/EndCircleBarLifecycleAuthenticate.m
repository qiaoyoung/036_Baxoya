...
- (void)updateTeamName{
    ...
    [self.groupnameView animationShow];
        ...
        self.groupnameView.speiceBackBlock = ^(NSString *groupName){
            ...
            [ElevatedNativeRecoverAllocator getWithUrl:Server_team_modifyname params:dict isShow:NO success:^(id responseObject) {
                [self reloadData];
...
- (void)dismissTeam {
    ...
    [self.groupAlartView animationShow];
        ...
        self.groupAlartView.speiceBackBlock = ^(NSString *Name){
            @strongify(self)
            [self didDismissTeam];
            [ElevatedNativeRecoverAllocator getWithUrl:Server_team_destory params:@{@"id":self.teamListManager.team.teamId} isShow:NO success:^(id responseObject) {
...
- (void)didOntransferWithLeave:(BOOL)isLeave {
    ...
    __block ContactSelectFinishBlock finishBlock =  ^(NSArray * memeber, NSString *name, UIImage *avater){
        ...
        [ElevatedNativeRecoverAllocator getWithUrl:Server_team_modifyowner params:dict isShow:NO success:^(id responseObject) {
            [wself reloadData];
        } failed:^(id responseObject, NSError *error) {
...
- (IdentifyMatrixRepaint *)aleartView{
    if(!_aleartView){
        _aleartView = [[IdentifyMatrixRepaint alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_groupnameView){
        _groupnameView = [[SliceBridgeGlobeResume alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (HighlightTraceAgainstNear *)groupNickNameView{
    if(!_groupNickNameView){
        _groupNickNameView = [[HighlightTraceAgainstNear alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (ConvolutionAxisRadiusEthereal *)groupAlartView{
    if(!_groupAlartView){
        _groupAlartView = [[ConvolutionAxisRadiusEthereal alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
