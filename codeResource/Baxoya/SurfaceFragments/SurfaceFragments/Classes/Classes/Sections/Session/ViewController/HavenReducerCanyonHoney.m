...
//    }
    ...
    self.header = [[EstimateClarityDepot alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, self.view.width, 0)];
...
- (void)onTapMenuItemMark:(ReplaceStopApply *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        if (error) {
            [FontVentureOrderly showErrorWithStatus:LangKey(@"收藏失败")];
            return;
        }
        [FontVentureOrderly showSuccessWithStatus:LangKey(@"收藏成功")];
    }];
}
...
- (void)onTapMenuItemTranslation:(ReplaceStopApply *)item
{
    ...
    [ElevatedNativeRecoverAllocator getWithUrl:Server_getNimCheckSum params:param isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
...
{
    if(!_translateView){
        _translateView = [[CombinerBesideRobustElevator alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (UnregisterOrbitCubitView *)reprotHisNextView
{
    if(!_reprotHisNextView){
        _reprotHisNextView  = [[UnregisterOrbitCubitView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
