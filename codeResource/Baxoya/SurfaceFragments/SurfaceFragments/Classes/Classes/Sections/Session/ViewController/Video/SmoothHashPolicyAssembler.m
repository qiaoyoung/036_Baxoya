...
@property (nonatomic, strong) ShuffleSenseShuffleTextured *videoSlider;  //滑动条
...
- (ShuffleSenseShuffleTextured *)videoSlider{
    if (!_videoSlider) {
        _videoSlider = [[ShuffleSenseShuffleTextured alloc] initWithFrame:CGRectMake(0, 0, 3*(SCREEN_WIDTH/4), 20)];
...
{
    if(!_loadingView){
        _loadingView = [[WithoutCollateStrategyAuthenticate alloc]initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT-SCREEN_TOP_HEIGHT-SCREEN_TABBAR_HEIGHT)];
    }
    return  _loadingView;
}
