...
@interface ExtendIterateStartFlat ()
...
@implementation ExtendIterateStartFlat

+ (ExtendIterateStartFlat *)sharedInstance {
    static ExtendIterateStartFlat *_sharedDevice = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedDevice = [[ExtendIterateStartFlat alloc] init];
    });
...
+ (NSString *)deviceIUUID {
    if (iuuid.length <= 0) {
        iuuid = [SerializeDistanceInspectorComponent passwordForService:SNUserDefault_Key_IUUID account:SNKey];
        if (iuuid.length <= 0) {
...
+ (void)updateDeviceIUUID {
    ...
    CFRelease(uuidRef);
    [SerializeDistanceInspectorComponent setPassword:iuuid forService:SNUserDefault_Key_IUUID account:SNKey];
}
