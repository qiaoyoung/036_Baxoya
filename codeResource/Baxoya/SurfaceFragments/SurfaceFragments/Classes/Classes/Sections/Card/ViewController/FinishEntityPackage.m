...
@interface FinishEntityPackage ()<NIMUserManagerDelegate,LayoutGroupDropForDelegate,OptimizeArcCountDelegate,UnregisterOrbitCubitDelegate>
...
@property (nonatomic, strong) UnregisterOrbitCubitUserView *reprotView;
@property (nonatomic, strong) DeriveVentureTrustedCollector *changeRemarksView;
...
@implementation FinishEntityPackage
...
//    [self setUpNav];
    ...
    [ElevatedNativeRecoverAllocator getWithUrl:Server_user_detail params:dict isShow:NO success:^(id responseObject) {
...
//    [FontVentureOrderly show];
    ...
    [ElevatedNativeRecoverAllocator getWithUrl:Server_group_getgroups params:nil isShow:NO success:^(id responseObject) {
        [self.loadingView animationClose];
//        [FontVentureOrderly dismissWithCompletion:^{
            NSDictionary *resultDict = (NSDictionary *)responseObject;
            NSString *code = [resultDict newStringValueForKey:@"code"];
            if (code.integerValue == 0) {
                ...
                LayoutGroupDropFor *dataPick = [[LayoutGroupDropFor alloc] initWithDelegate:self dataDict:dataDict selectedDict:nil jsonNode:@"name"];
                dataPick.tag = 500;
                [dataPick show];
            }
//        }];
...
#pragma mark - LayoutGroupDropForDelegate
...
-(void)setingGroup{
    ...
    [ElevatedNativeRecoverAllocator getWithUrl:Server_group_addgroupuser params:dict isShow:YES success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
...
- (DeriveVentureTrustedCollector *)changeRemarksView
{
    if(!_changeRemarksView){
        _changeRemarksView = [[DeriveVentureTrustedCollector alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (UnregisterOrbitCubitUserView *)reprotView
{
    if(!_reprotView){
        _reprotView  = [[UnregisterOrbitCubitUserView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
-(void)didTouchSubmitButton:(NSString *)reason
{
    NSMutableDictionary *dict = @{}.mutableCopy;
    dict[@"contact"] = reason?:@"";
    [ElevatedNativeRecoverAllocator postWithUrl:Server_other_feedback params:dict isShow:NO success:^(id responseObject) {
    } failed:^(id responseObject, NSError *error) {
    }];
...
{
    if(!_reprotNextView){
        _reprotNextView  = [[UnregisterOrbitCubitNextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotBlackView){
        _reprotBlackView  = [[UnregisterOrbitCubitBlackView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (UnregisterOrbitCubitDeleteView *)reprotDeleteView
{
    if(!_reprotDeleteView){
        _reprotDeleteView  = [[UnregisterOrbitCubitDeleteView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
