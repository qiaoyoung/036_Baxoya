...
// __CLOSE_PRINT__
//
//  NSData+ReplaceMysticViewFacadeFlow.m
//  NIM
//
...
- (NSData *)replacementCalm:(NSString *)key agent:(NSString *)vector {
...
- (NSData *)resume:(NSString *)key relatedReliability:(NSString *)vector {
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    //: assert(key.length == 32);
    assert(key.length == 32);
    //: char keyPtr[kCCKeySizeAES256 + 1];
...
void boundElegant(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)margin:(NSString *)key {
    ...
    boundElegant(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
