...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [FontVentureOrderly showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
...
- (UpgradeAnimatorEnforceDale *)selectIndicatorView{
    ...
    _selectIndicatorView = [[UpgradeAnimatorEnforceDale alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
...
- (UntilVentureRotate *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[UntilVentureRotate alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
