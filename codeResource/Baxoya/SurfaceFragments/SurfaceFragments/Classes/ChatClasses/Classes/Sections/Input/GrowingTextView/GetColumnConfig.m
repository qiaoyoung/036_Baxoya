...
//: @interface GetColumnConfig()
@interface GetColumnConfig()

//: @property (nonatomic,assign) BOOL displayPlaceholder;
@property (nonatomic,assign) BOOL organic;

//: @end
@end

//: @implementation GetColumnConfig
@implementation GetColumnConfig
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSAttributedString *attributedString = self.song;
...
- (instancetype)initWithFrame:(CGRect)frame textContainer:(NSTextContainer *)textContainer
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(evaluations:) name:UITextViewTextDidChangeNotification object:self];
    }
    //: return self;
    return self;
}
...
- (void)setSong:(NSAttributedString *)placeholderAttributedText
{
    //: _placeholderAttributedText = placeholderAttributedText;
    _song = placeholderAttributedText;
    //: [self setNeedsDisplay];
    [self setNeedsDisplay];
}

//: - (void)textDidChangeNotification:(NSNotification *)notification
- (void)evaluations:(NSNotification *)notification
{
...
- (void)setOrganic:(BOOL)displayPlaceholder
{
    //: BOOL oldValue = _displayPlaceholder;
    BOOL oldValue = _organic;
    //: _displayPlaceholder = displayPlaceholder;
    _organic = displayPlaceholder;
    //: if (oldValue != self.displayPlaceholder) {
    if (oldValue != self.organic) {
        //: [self setNeedsDisplay];
        [self setNeedsDisplay];
    }
}
...
- (void)acrossEntity
{
    ...
    self.organic = self.text.length == 0;
}
