...
//: @implementation NSString (HeightBrainSetWinter)
@implementation NSString (HeightBrainSetWinter)

//: static char NIMKitStringJsonDictionaryAddress;
static char componentCenterPreference;
//: - (NSDictionary *)nimkit_jsonDict
- (NSDictionary *)mirror
{
    //: NSDictionary *dict = [objc_getAssociatedObject(self, &NIMKitStringJsonDictionaryAddress) copy];
    NSDictionary *dict = [objc_getAssociatedObject(self, &componentCenterPreference) copy];
    //: if (dict == nil) 
    if (dict == nil) //解析过一次后就缓存解析结果，避免多次解析
    {
        ...
        objc_setAssociatedObject(self,&componentCenterPreference,dict,OBJC_ASSOCIATION_COPY);
    }
    //: return dict;
    return dict;
...
//: @implementation NSDictionary (HeightBrainSetWinter)
@implementation NSDictionary (HeightBrainSetWinter)
